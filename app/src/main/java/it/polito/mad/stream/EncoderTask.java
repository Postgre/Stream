...
public class EncoderTask extends AsyncTask<Void, VideoChunksNew.Chunk, Void> {
    ...
    private VideoChunksNew mRawFrames = new VideoChunksNew();
        ...
        while (!outputDone) {
            if (!inputDone) {
                ...
                if (framesCounter == NUM_FRAMES) {
                    ...
                    byte[] previewData = mRawFrames.getNextChunk().data;
            ...
            if (outputStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                ...
                VideoChunksNew.Chunk c =
                        new VideoChunksNew.Chunk(encodedArray, info.flags, info.presentationTimeUs);
                publishProgress(c);
